...
namespace Registry.Other
{
    ...
    public static class Helpers
    {
        ...
        public enum SidTypeEnum
        {
            ...
            [Description("S-1-2-1: A group that includes users who are logged on to the physical console. This SID can be used to implement security policies that grant different rights based on whether a user has been granted physical access to the console.")]
            ConsoleLogon,

            [Description("S-1-3-0: A placeholder in an inheritable access control entry (ACE). When the ACE is inherited, the system replaces this SID with the SID for the object's creator.")]
            CreatorOwner,

            [Description("S-1-3-1: A placeholder in an inheritable ACE. When the ACE is inherited, the system replaces this SID with the SID for the primary group of the object's creator.")]
            CreatorGroup,

            [Description("S-1-3-2: A placeholder in an inheritable ACE. When the ACE is inherited, the system replaces this SID with the SID for the object's owner server.")]
            OwnerServer,

            [Description("S-1-3-3: A placeholder in an inheritable ACE. When the ACE is inherited, the system replaces this SID with the SID for the object's group server.")]
            GroupServer,

            [Description("S-1-3-4: A group that represents the current owner of the object. When an ACE that carries this SID is applied to an object, the system ignores the implicit READ_CONTROL and WRITE_DAC permissions for the object owner.")]
            OwnerRights,
            ...
            [Description("S-1-5-5-x-y: A logon session. The X and Y values for these SIDs are different for each logon session and are recycled when the operating system is restarted.")]
            LogonId,
            ...
            [Description("S-1-5-9: A group that includes all domain controllers in a forest that uses an Active Directory directory service.")]
            EnterpriseDomainControllers,

            [Description("S-1-5-10: A placeholder in an inheritable ACE on an account object or group object in Active Directory. When the ACE is inherited, the system replaces this SID with the SID for the security principal that holds the account.")]
            PrincipalSelf,

            [Description("S-1-5-11: A group that includes all users whose identities were authenticated when they logged on.")]
            AuthenticatedUsers,

            [Description("S-1-5-12: This SID is used to control access by untrusted code. ACL validation against tokens with RC consists of two checks, one against the token's normal list of SIDs and one against a second list (typically containing RC - the RESTRICTED_CODE token - and a subset of the original token SIDs). Access is granted only if a token passes both tests. Any ACL that specifies RC must also specify WD - the EVERYONE token. When RC is paired with WD in an ACL, a superset of EVERYONE, including untrusted code, is described.")]
            RestrictedCode,

            [Description("S-1-5-13: A group that includes all users who have logged on to a Terminal Services server.")]
            TerminalServerUser,

            [Description("S-1-5-14: A group that includes all users who have logged on through a terminal services logon.")]
            ...
            [Description("S-1-5-21-<root domain>-498: A universal group containing all read-only domain controllers in a forest.")]
            EnterpriseReadonlyDomainControllers,

            [Description("S-1-5-21-0-0-0-496: Device identity is included in the Kerberos service ticket. If a forest boundary was crossed, then claims transformation occurred.")]
            CompoundedAuthentication,

            [Description("S-1-5-21-0-0-0-497: Claims were queried for in the account's domain, and if a forest boundary was crossed, then claims transformation occurred.")]
            ClaimsValid,

            [Description("S-1-5-21-<machine>-500: A user account for the system administrator. By default, it is the only user account that is given full control over the system.")]
            Administrator,

            [Description("S-1-5-21-<machine>-501: A user account for people who do not have individual accounts. This user account does not require a password. By default, the Guest account is disabled.")]
            Guest,

            [Description("S-1-5-21-<domain>-512: A global group whose members are authorized to administer the domain. By default, the DOMAIN_ADMINS group is a member of the Administrators group on all computers that have joined a domain, including the domain controllers. DOMAIN_ADMINS is the default owner of any object that is created by any member of the group.")]
            DomainAdmins,

            [Description("S-1-5-21-<domain>-513: A global group that includes all user accounts in a domain.")]
            DomainUsers,

            [Description("S-1-5-21-<domain>-514: A global group that has only one member, which is the built-in Guest account of the domain.")]
            DomainGuests,

            [Description("S-1-5-21-<domain>-515: A global group that includes all clients and servers that have joined the domain.")]
            DomainComputers,

            [Description("S-1-5-21-<domain>-516: A global group that includes all domain controllers in the domain.")]
            DomainDomainControllers,

            [Description("S-1-5-21-<domain>-517: A global group that includes all computers that are running an enterprise certification authority. Cert Publishers are authorized to publish certificates for User objects in Active Directory.")]
            CertPublishers,

            [Description("S-1-5-21-<root-domain>-518: A universal group in a native-mode domain, or a global group in a mixed-mode domain. The group is authorized to make schema changes in Active Directory.")]
            SchemaAdministrators,

            [Description("S-1-5-21-<root-domain>-519: A universal group in a native-mode domain, or a global group in a mixed-mode domain. The group is authorized to make forestwide changes in Active Directory, such as adding child domains.")]
            EnterpriseAdmins,

            [Description("S-1-5-21-<domain>-520: A global group that is authorized to create new Group Policy Objects in Active Directory.")]
            ...
            [Description("S-1-5-21-<domain>-522: A global group that includes all domain controllers in the domain that may be cloned.")]
            CloneableControllers,

            [Description("S-1-5-21-<domain>-525: A global group that are afforded additional protections against authentication security threats. For more information, see [MS-APDS] and [MS-KILE].")]
            ProtectedUsers,

            [Description("S-1-5-21-<domain>-553: A domain local group for Remote Access Services (RAS) servers. Servers in this group have Read Account Restrictions and Read Logon Information access to User objects in the Active Directory domain local group.")]
            RasServers,

            [Description("S-1-5-32-544: A built-in group. After the initial installation of the operating system, the only member of the group is the Administrator account. When a computer joins a domain, the Domain Administrators group is added to the Administrators group. When a server becomes a domain controller, the Enterprise Administrators group also is added to the Administrators group.")]
            BuiltinAdministrators,

            [Description("S-1-5-32-545: A built-in group. After the initial installation of the operating system, the only member is the Authenticated Users group. When a computer joins a domain, the Domain Users group is added to the Users group on the computer.")]
            BuiltinUsers,

            [Description("S-1-5-32-546: A built-in group. The Guests group allows users to log on with limited privileges to a computer's built-in Guest account.")]
            BuiltinGuests,

            [Description("S-1-5-32-547: A built-in group. Power users can perform the following actions: Create local users and groups, Modify and delete accounts that they have created, Remove users from the Power Users, Users, and Guests groups, Install programs, Create, manage, and delete local printers, Create and delete file shares.")]
            PowerUsers,

            [Description("S-1-5-32-548: A built-in group that exists only on domain controllers. Account Operators have permission to create, modify, and delete accounts for users, groups, and computers in all containers and organizational units of Active Directory except the Built-in container and the Domain Controllers OU. Account Operators do not have permission to modify the Administrators and Domain Administrators groups, nor do they have permission to modify the accounts for members of those groups.")]
            AccountOperators,

            [Description("S-1-5-32-549: A built-in group that exists only on domain controllers. Server Operators can perform the following actions: Log on to a server interactively, Create and delete network shares, Start and stop services, Back up and restore files, Format the hard disk of a computer, Shut down the computer")]
            ServerOperators,

            [Description("S-1-5-32-550: A built-in group that exists only on domain controllers. Print Operators can manage printers and document queues.")]
            PrinterOperators,

            [Description("S-1-5-32-551: A built-in group. Backup Operators can back up and restore all files on a computer, regardless of the permissions that protect those files.")]
            BackupOperators,

            [Description("S-1-5-32-552: A built-in group that is used by the File Replication Service (FRS) on domain controllers.")]
            Replicator,

            [Description("S-1-5-32-554: A backward compatibility group that allows read access on all users and groups in the domain.")]
            AliasPrew2Kcompacc,

            [Description("S-1-5-32-555: An alias. Members of this group are granted the right to log on remotely.")]
            RemoteDesktop,

            [Description("S-1-5-32-556: An alias. Members of this group can have some administrative privileges to manage configuration of networking features.")]
            NetworkConfigurationOps,

            [Description("S-1-5-32-557: An alias. Members of this group can create incoming, one-way trusts to this forest.")]
            ...
            [Description("S-1-5-32-559: An alias. Members of this group have remote access to schedule the logging of performance counters on this computer.")]
            PerflogUsers,

            [Description("S-1-5-32-560: An alias. Members of this group have access to the computed tokenGroupsGlobalAndUniversal attribute on User objects.")]
            ...
            [Description("S-1-5-32-562: An alias. A group for COM to provide computer-wide access controls that govern access to all call, activation, or launch requests on the computer.")]
            DistributedComUsers,
            ...
            [Description("S-1-5-32-573: A built-in local group. Members of this group can read event logs from the local machine.")]
            EventLogReaders,

            [Description("S-1-5-32-574: A built-in local group. Members of this group are allowed to connect to Certification Authorities in the enterprise.")]
            ...
            [Description("S-1-5-32-577: A group that allows members to access WMI resources over management protocols (such as WS-Management via the Windows Remote Management service).")]
            RdsManagementServers,

            [Description("S-1-5-32-578: A group that gives members access to all administrative features of Hyper-V.")]
            HyperVAdmins,

            [Description("S-1-5-32-579: A local group that allows members to remotely query authorization attributes and permissions for resources on the local computer.")]
            AccessControlAssistanceOps,

            [Description("S-1-5-32-580: Members of this group can access Windows Management Instrumentation (WMI) resources over management protocols (such as WS-Management [DMTF-DSP0226]). This applies only to WMI namespaces that grant access to the user.")]
            RemoteManagementUsers,

            [Description("S-1-5-33: A SID that allows objects to have an ACL that lets any service process with a write-restricted token to write to the object.")]
            WriteRestrictedCode,

            [Description("S-1-5-64-10: A SID that is used when the NTLM authentication package authenticated the client.")]
            NtlmAuthentication,

            [Description("S-1-5-64-14: A SID that is used when the SChannel authentication package authenticated the client.")]
            SchannelAuthentication,

            [Description("S-1-5-64-21: A SID that is used when the Digest authentication package authenticated the client.")]
            DigestAuthentication,

            [Description("S-1-5-65-1: A SID that indicates that the client's Kerberos service ticket's PAC contained a NTLM_SUPPLEMENTAL_CREDENTIAL structure (as specified in [MS-PAC] section 2.6.4).")]
            ...
            [Description("S-1-5-114: A group that includes all users who are local accounts and members of the administrators group.")]
            ...
            [Description("S-1-18-1: A SID that means the client's identity is asserted by an authentication authority based on proof of possession of client credentials.")]
        ...
        public static T GetEnumValueFromDescription<T>(string description)
        {
            ...
            var field = fields.SelectMany(f => f.GetCustomAttributes(typeof(DescriptionAttribute), false), (f, a) => new
            {
                Field = f,
                Att = a
